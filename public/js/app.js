...
socket.on('gameOver', (turn, win) => {
    ...
    if (win) {
        if (board.orientation().includes(turn)) {
            statusEl.textContent = "ゲームに敗北しました。下部の[ゲームを退出]から終了できます"
        }
        else {
            statusEl.textContent = "ゲームに勝利しました！下部の[ゲームを退出]から終了できます"
        }
    }
    else {
        statusEl.value = '引き分け！下部の[ゲームを退出]から終了できます'
    }
})
...
socket.on('disconnectedStatus', () => {
    ...
    messageEl.textContent = '対戦相手がゲームを離脱しました'
})
...
joinButtonEl.addEventListener('click', (e) => {
    ...
    if (!user || !room) {
        ...
        messageEl.textContent = "他の人の参加を待っています..."
    }
})
